...
$error   = '';
$success = isset($_GET['password_changed']);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $password = $_POST['password'] ?? '';
    $remember = isset($_POST['remember']);

    $stmt = $pdo->prepare('SELECT setting_value FROM config WHERE setting_name = ?');
    ...
    if ($row && password_verify($password, $row['setting_value'])) {
        ...
        if ($remember) {
            try {
                ...
                $pdo->prepare('INSERT INTO auth_tokens (token_hash, created_at) VALUES (?, NOW())')
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .login-header {
            background: var(--accent); padding: 2rem 2rem 1.75rem; text-align: center;
        }
        .login-logo {
        ...
        .login-header h1 { font-size: 1.15rem; font-weight: 600; color: #fff; letter-spacing: -.01em; }
        .login-header p  { font-size: .8rem; color: rgba(255,255,255,.65); margin: .2rem 0 0; }
        ...
        .alert {
            ...
            font-size: .845rem; margin-bottom: 1.1rem; line-height: 1.4;
        }
        ...
        .field label { display: block; font-size: .82rem; font-weight: 500; color: var(--text-2); margin-bottom: .38rem; }

        .input-wrap { position: relative; }
        .input-wrap i {
            position: absolute; left: .78rem; top: 50%;
            transform: translateY(-50%); color: var(--text-3); font-size: .82rem; pointer-events: none;
        }
        .input-wrap input {
            width: 100%; padding: .55rem .75rem .55rem 2.3rem;
            border: 1px solid var(--border); border-radius: var(--radius-sm);
            font-family: 'DM Sans', sans-serif; font-size: .9rem; color: var(--text);
            background: var(--surface); transition: border-color .15s, box-shadow .15s; appearance: none;
        }
        ...
        .remember span { font-size: .82rem; color: var(--text-2); user-select: none; }
        ...
        <form method="POST">
            <div class="field">
                ...
                <div class="input-wrap">
                    ...
                    <input type="password" id="password" name="password" placeholder="Введіть пароль…" required autofocus>
                </div>
            </div>
...
<div class="login-footer">&copy; <?= date('Y') ?> Система куратора</div>
